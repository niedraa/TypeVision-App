...
import OpponentCursors from '../components/OpponentCursors';
import LiveTypingIndicator from '../components/LiveTypingIndicator';
import TextProgressMap from '../components/TextProgressMap';
...
const MultiplayerGameScreen = ({ roomData, onGameComplete, onBack }) => {
  ...
  const renderTextWithOpponentProgress = () => {
    ...
    return gameText.split('').map((char, index) => {
      ...
      // Vérifier si des adversaires sont à cette position
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <TextProgressMap 
          gameText={gameText}
          players={players}
          currentPlayerId={globalMultiplayerService.currentPlayerId}
